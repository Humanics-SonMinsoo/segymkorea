...
2. **소유 확인** — 아래 중 **하나만** 성공해도 됩니다.
   - **HTML 파일**: `https://segymkorea.com/naver603a069df921dcb5566c6df25e62567d.html` — `next.config.js`의 `beforeFiles` 리라이트로 `/api/naver-site-verification`에서 HTML을 내려보냄(404 방지). `public/` 동명 파일은 보조. **서치어드바이저에 등록한 호스트**(루트/www)와 브라우저에서 여는 주소가 같아야 함.
