 ...
 */
export function ProductSpecSheet({ locale = 'ko' }: Props) {
  const en = locale === 'en'
  ...
  return (
    <div className="space-y-8 sm:space-y-10">
      <div className="rounded-2xl overflow-hidden bg-white border border-gray-200 shadow-sm">
        <div className="px-5 sm:px-8 py-5 sm:py-6 border-b border-gray-100 bg-gray-50/80">
          ...
          <h3 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">
            {en ? 'Base specifications' : '기본 사양'}
          </h3>
          <p className="mt-2 text-sm text-gray-600 max-w-2xl leading-relaxed">
            {en ? (
              <>
                Specs below are based on the Standard configuration (Wide, black frame, 2D barbell).
                <br />
                Side covers, Performance-level upgrades, height options, and other items not listed in the table are
                covered under Optional upgrades below.
              </>
            ) : (
              <>
        ...
        <div className="overflow-x-auto scrollbar-hide">
          <table className="w-full min-w-[320px] text-left text-sm border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              <BaseRow label={en ? 'Max load' : '최대 하중'} value="170kg" />
              <BaseRow label={en ? 'Max speed' : '최대 속도'} value="2.4m/s" />
              <BaseRow
                label={en ? 'Dimensions' : '제품 크기'}
                value={
                  <>
                    W 2,200 × D 1,600 × H 2,100 mm
                    <span className="block text-xs text-gray-500 mt-1">
                      {en ? 'Wide type · standard height' : '와이드 타입 기본 전고'}
                    </span>
                  </>
                }
              />
              <BaseRow
                label={en ? 'Barbell (flex bars)' : '바벨 (탄력봉)'}
                value={
                  en ? (
                    <span>
                      <strong className="text-gray-900">2D</strong>: left and right flex bars move up and down on the
                      rails <strong className="text-gray-900">independently</strong>.
                      <br />
                      The two sides are not locked to the same height, so single-side work and left–right strength
                      differences can be reflected.
                    </span>
                  ) : (
                    <span>
              ...
              />
              <BaseRow label={en ? 'Frame color' : '프레임 색상'} value={en ? 'Black' : '블랙'} />
              <BaseRow
                label={en ? 'Touch display' : '터치 디스플레이'}
                value={
                  en
                    ? '21.5″ touch monitor (integrated) included as standard'
                    : '21.5″ 터치 모니터 (일체형) 기본 제공'
                }
              />
            </tbody>
          </table>
      ...
      <div className="rounded-2xl border border-gray-200 bg-gray-50/50 p-6 sm:p-8 space-y-6">
        <h4 className="card-title text-lg sm:text-xl text-gray-900">
          {en ? 'Optional upgrades' : '선택 옵션 안내'}
        </h4>
        <p className="text-sm text-gray-600 leading-relaxed">
          {en ? (
            <>
              The items below can be selected in addition to the base configuration.
              <br />
              Availability and detailed specs are confirmed during the inquiry process.
            </>
          ) : (
            <>
        ...
        <ul className="space-y-5 text-sm text-gray-700">
          <li className="leading-relaxed">
            <span className="font-semibold text-primary block mb-1">
              {en ? 'Side covers' : '측면 커버'}
            </span>
            {en
              ? 'Choose transparent or signature purple. Both options are offered under the same terms.'
              : (
                <>
          ...
          <li className="leading-relaxed">
            <span className="font-semibold text-primary block mb-1">
              {en ? 'Performance-level upgrade' : '퍼포먼스 레벨 업그레이드'}
            </span>
            {en ? (
              <>
                Step up from the base max load (<strong className="text-gray-900">170kg</strong>) to{' '}
                <strong className="text-gray-900">Performance level</strong>.
                <br />
                When applied, max load extends up to about <strong className="text-gray-900">260kg</strong>.
              </>
            ) : (
              <>
          ...
          <li className="leading-relaxed">
            <span className="font-semibold text-primary block mb-1">
              {en ? 'Overall height · Giant height' : '전고(높이), 자이언트 높이'}
            </span>
            {en ? (
              <>
                The Giant height option raises overall height up to{' '}
                <strong className="text-gray-900">2,400mm</strong>.
                <br />
                Width and depth stay the same as the Wide base, while height can be matched to ceiling clearance and
                site conditions.
              </>
            ) : (
              <>
      ...
      <div className="rounded-2xl border border-primary/25 bg-gradient-to-br from-primary-muted/40 to-white p-6 sm:p-8 text-center">
        <p className="text-sm sm:text-base text-gray-700 leading-relaxed max-w-xl mx-auto mb-5">
          {en ? (
            <>
              Looking at the specs, you may also want to know about{' '}
              <strong className="text-gray-900">deployment cost or monthly payment options</strong>.
              <br />
              Reach out and our team will walk you through pricing for the Smith Robot (SR).
            </>
          ) : (
            <>
